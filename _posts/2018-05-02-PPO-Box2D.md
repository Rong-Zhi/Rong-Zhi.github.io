...
We couldn't find the reasons based on current results. But if we take a look at the environment code about how to define the reward function, things get clear
...
The shaping function could be written as \\[ -100(x^2+y^2)^2-100(V_x^2+V_y^2)^2-\mid100\theta\mid+10L_l+10L_r \\], the starting point is always at coordinates(0,1) with random velocity, theta=0, L_l=0, L_r=0. Episode finishes if the lander crushes or comes to rest, receiving -100 or +100 reward. We firstly change reward=-1 for the first case, and get the following result:
<img src="/images/PPO-lunarlander/imgs/ppo2-llm-1.png" height="500" width="700">

Without the punishment for crush, the lander still couldn't get the optimal solution, which means it could learn to avoid crush. Now let's see the other case: change reward=+1 when it comes to rest.
